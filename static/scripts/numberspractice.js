...
function userSubmittedAnswer() {
    if (state == STATE_PROMPT) {
        ...
        if (mode == MODE_TRANSLATE) {
            correct = input == currentNumber.english || input == currentNumber.english.replaceAll('-', ' ') || input == currentNumber.number || input.replace(' ', '\u202F') == currentNumber.number.toLocaleString('fr-FR');
        } else {
