...
def get_known_terms_in_category(category_id):
    ...
    connection = get_database()
    cursor = connection.cursor()
    cursor.execute('select french, english, englishAlts, frenchAlts, id, gender, difficulty from term, userKnowsTerm where id = termid and userid = ? and id in (select termid from termInCategory where categoryid=?) and id in (select termid from userKnowsTerm where userid=?)', (user_id, category_id, user_id))
    terms = []
