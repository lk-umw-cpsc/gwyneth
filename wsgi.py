...
def record_attempt(number, correct):
    ...
    if correct:
        ...
        cursor.execute('update userKnowsNumber set difficulty = difficulty - 1 where userid=? and number=?', (id, number))
    else:
        # cursor.execute('update userKnowsNumber set difficulty = min(difficulty + 2, 7) where userid=? and number=?', (id, number))
        cursor.execute('update userKnowsNumber set difficulty = difficulty + 2 where userid=? and number=?', (id, number))
    connection.commit()
    connection.close()